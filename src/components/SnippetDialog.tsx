...
const SnippetDialog = ({ value, language }: SnippetDialogProps) => {
	...
	const { mutateAsync: saveSnippet, isLoading: isSavingSnippet } =
		api.snippet.saveSnippet.useMutation();
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Enter a name, so you can identify your snippet later. Optionally you
						can add a description to further seperate your snippet
					</DialogDescription>
					<div className="">
						<Label>Name</Label>
						<Input
					...
					<div className="">
						<Select>
							...
							<SelectTrigger>
								<SelectValue
									placeholder="Select a language or format"
					...
					<div className="">
						...
						<Textarea
							value={value}
							disabled
							placeholder="Snippet value wil be visible here"
						/>
					</div>
					...
					<DialogFooter>
						<Button
							...
							variant="default"
							className="w-full"
							disabled={isSavingSnippet || !snippetName || !value}
						>
							<Save className="mr-2 h-4 w-4" /> Save Snippet
						</Button>
					</DialogFooter>
