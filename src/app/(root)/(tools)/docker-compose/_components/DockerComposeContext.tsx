...
import { type ChangeEvent, type ReactNode, createContext, useContext, useEffect, useState } from "react";
/* eslint-disable @typescript-eslint/no-unsafe-member-access */
/* eslint-disable @typescript-eslint/no-unsafe-call */
import type { DockerService, GetStringKeys } from "@/types/docker-service";
...
const DockerComposeContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const parseDockerCommand = (cmd: string) => {
    ...
    const env = getValueFromCommand(cmd, { prefixes: ["-e", "--env"], seperator: "=" });
    const labels = getValueFromCommand(cmd, { prefixes: ["-l", "--label"], seperator: "=" });
    const ports = getValueFromCommand(cmd, { prefixes: ["-p", "--port"] });
    const volumes = getValueFromCommand(cmd, { prefixes: ["-v", "--volume"] });
    ...
    const restart = getConfigFromCommand(cmd, { prefixes: ["--restart"] });

    const network = getConfigFromCommand(cmd, { prefixes: ["--net"] });

    const object: Record<
      string,
      { image: string; container_name: string; ports?: string[]; volumes?: string[]; environment?: string[]; labels?: string[]; restart?: string; network_mode?: string }
    > = {
      [name]: {
        image,
        container_name: name,
  ...
  const getConfigFromCommand = (cmd: string, { prefixes }: { prefixes: string[] }) => {
  ...
  const getValueFromCommand = (cmd: string, { prefixes, seperator = ":" }: { prefixes: string[]; seperator?: string }): { id: string; label: string; value: string }[] => {
    const contains = prefixes.some((prefix) => cmd.includes(prefix));
  ...
  const generateDockerComposeFromServices = () => {
    const serviceJson = services.map((service) => {
      ...
      const mainObject: Record<string, { image: string; user: string; container_name: string; ports?: string[]; volumes?: string[]; environment?: string[]; labels?: string[] }> = {
        [name]: {
          image: service.image,
          user: "1000",
          // working_dir: null,
