...
const useDockerCompose = () => {
	if (!dockerComposeContext) {
		throw new Error(
			"useDockerComposeContext must be used within a DockerComposeContextProvider",
		);
	}
...
const DockerComposeContextProvider = ({
	children,
}: { children: ReactNode }) => {
	...
	const parseDockerCommand = (cmd: string) => {
		...
		if (volumes.length > 0) {
			service.volumes = volumes.map(
				(volume) => `${volume.label}:${volume.value}`,
			);
		}
	...
	const generateDockerComposeFromServices = () => {
		const serviceJson = services.map((service) => {
			...
			> = {
				[name]: {
					...
					ports: service.ports
						.filter((port) => port.external !== "" || port.internal !== "")
						.map((port) => `${port.external}:${port.internal}`),
					volumes: service.volumes
						.filter((port) => port.external !== "" || port.internal !== "")
						.map((volume) => `${volume.external}:${volume.internal}`),
					environment: service.env
						.filter((port) => port.label !== "" || port.value !== "")
						.map((env) => `${env.label}=${env.value}`),
			...
			if (
				...
				serviceObject.volumes.length === 0
			) {
				serviceObject.volumes = undefined;
			}
			...
			if (
				...
				serviceObject.environment.length === 0
			) {
				serviceObject.environment = undefined;
			}
		...
		const allServices = Object.assign({}, ...serviceJson) as Record<
			string,
			unknown
		>;
	...
	const handleNameOfServiceChange = (
		e: ChangeEvent<HTMLInputElement>,
		name: string,
		key: GetStringKeys,
	) => {
	...
	const addEnvToService = (name: string) => {
		...
		service.env.push({
			label: "",
			value: "",
			id: service.env.length.toString(),
		});
		setServices([...services]);
	};
	...
	const addLabelToService = (name: string) => {
		...
		service.labels.push({
			label: "",
			value: "",
			id: service.labels.length.toString(),
		});
		setServices([...services]);
	};
	...
	const addPortToService = (name: string) => {
		...
		service.ports.push({
			external: "",
			internal: "",
			id: service.ports.length.toString(),
		});
		setServices([...services]);
	};
	...
	const addVolumeToService = (name: string) => {
		...
		service.volumes.push({
			external: "",
			internal: "",
			id: service.volumes.length.toString(),
		});
		setServices([...services]);
	};
