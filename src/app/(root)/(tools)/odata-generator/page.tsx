...
const OdataGenerator = () => {
  ...
  useEffect(() => {
    setfilterValue([{ id: 0, key: "", type: "default", comparator: "eq", value: [""], valueType: "string" }]);
  ...
  const addFilter = (type?: string) => {
    setfilterValue((prev) => {
      const newFilter = prev?.concat({ id: prev?.length || 0, value: [""], key: "", type: type ?? "default", comparator: "eq", valueType: "string" });
      return newFilter;
    });
  };
  ...
  const deleteOptionalFilter = (id: number, optionalId: number) => {
    setfilterValue((prev) => {
      const newFilter = prev?.map((filter) => {
        if (filter.id === id) {
          return { ...filter, optionalComparisons: filter.optionalComparisons?.filter((filter) => filter.id !== optionalId) };
        }
        return filter;
      });
      return newFilter;
  ...
  return (
    <HeaderLayout title="ODATA Generator" desc="Generate ODATA queries with ease." toolId={12}>
      <ToolButtons
        first={
