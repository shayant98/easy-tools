			...
			<SheetContent className="flex h-screen flex-col border-none">
				...
				<div className="flex h-full flex-col gap-5">
					<div className="relative flex h-full flex-col gap-5 overflow-scroll">
						<div className="space-y-2">
							...
							{node.type === "archParentNode" && (
								<div className="mt-10 flex flex-row items-center justify-between rounded-lg border border-muted p-3 shadow-sm">
									...
									<Switch
										checked={(node as Node<NodeParentData>).data.showLabel}
										onCheckedChange={(val) =>
											handleDataChange(val, "showLabel")
										}
									/>
								</div>
							)}
							...
							{node.type !== "archParentNode" && (
								<>
									...
									<ColorPicker
										background={(node as Node<NodeData>).data.color ?? ""}
										setBackground={(v) => handleDataChange(v, "color")}
									/>
						...
						{node.type !== "archParentNode" && (
							<ServicePicker node={node as Node<NodeData>} />
						)}
					</div>
				</div>
			</SheetContent>
