...
import { type Node, getNodesBounds, useReactFlow } from "reactflow";
...
const NodeContextMenu = () => {
	...
	const groupNodes = () => {
		const hasGroupedNodes = selectedNodes.filter(
			(node) => node.parentNode !== undefined,
		);
		const hasParents = selectedNodes.filter(
			(node) => node.type === "archParentNode",
		);
		...
		const numberOfGroups = nodes.filter(
			(node) => node.type === "archParentNode",
		);
		...
		const updatedNodes = getNodes().map((node) => {
			const hasNode = selectedNodes.filter(
		...
		const parentHasChildren = updatedNodes.filter(
	...
	const removeFromParent = (nodeId: string) => {
		...
		const childrenOfParent = nodes.filter(
			(node) => node.parentNode === currentNode?.parentNode,
		);
		...
		if (childrenOfParent.length === 1) {
			updatedNodesList = updatedNodesList.filter(
				(node) => node.id !== currentNode?.parentNode,
			);
		...
		return (
			<ContextMenuContent className="w-64">
				...
				{selectedNodes[0]?.parentNode !== undefined && (
					<ContextMenuItem
						onClick={() => removeFromParent(selectedNodes[0]?.id ?? "")}
						inset
					>
						Extract from parent
					</ContextMenuItem>
